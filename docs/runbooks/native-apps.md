...
`pnpm icons` draws every web and store icon from `src/components/brand/clover.ts`, the same geometry
the in-app mark uses, and writes them to `public/icons/` — including `icon-1024.png`, the square
master the platform tooling and the store listings are generated from.

The bundled native icon sets are **not** regenerated by that script, because `.icns` and `.ico` need
encoders `sharp` does not have. After changing the mark, regenerate them from the master:

```bash
pnpm icons                      # refresh public/icons, incl. the icon-1024.png master

# desktop: the tauri script lives in apps/desktop, not the root
pnpm --filter clover-desktop exec tauri icon ../../public/icons/icon-1024.png

# mobile
npx @capacitor/assets generate --iconOnly \
  --assetPath public/icons --iosProject apps/mobile/ios --androidProject apps/mobile/android
```

Check two things afterwards, because both fail silently and only show up on a device:

- **Android adaptive icons** get cropped to a circle of 80% diameter. `icon-maskable-512.png`
  already accounts for that (`CLOVER_SCALE.maskable`), so use it rather than the square master
  wherever a maskable source is asked for.
- **iOS rejects icons with an alpha channel.** The generated PNGs are opaque; if you hand-edit one,
  flatten it.
