...
type Row = { key: keyof Capabilities; name: string; enabled: string; fallback: string; env: string[]; note?: string };
...
const ROWS: Row[] = [
  ...
  { key: "segmentation", name: "Studio backgrounds", enabled: "A segmentation provider cuts the item out so a new background can be drawn around it.", fallback: "Enhancement only: exposure and colour, no background replacement.", env: ["STUDIO_SEGMENTATION_PROVIDER", "PHOTOROOM_API_KEY", "REMOVEBG_API_KEY", "RUNPOD_API_KEY", "RUNPOD_SEGMENT_ENDPOINT_ID"], note: "Set the provider to photoroom, removebg or runpod and the matching key(s)." },
            ...
            <div key={r.key} className="grid min-w-0 grid-cols-1 gap-2 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
              ...
              <div className="min-w-0 sm:max-w-xs sm:text-right">
                ...
                <div className="mt-0.5 flex flex-wrap gap-1 sm:justify-end">
                  {r.env.map((e) => (
                    <code key={e} className="break-all rounded-[4px] bg-surface-sunken px-1.5 py-0.5 font-mono text-[11px] text-secondary">
                      {e}
                    </code>
                  ))}
                </div>
