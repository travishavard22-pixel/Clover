...
/**
 * POST /api/items/[id]/profile/reanalyze → { jobId, reused }
 * Re-runs the full ANALYZE_ITEM pipeline. Fields the seller verified by hand are preserved by the
 * handler. A queued or running job for this item is returned instead of starting a second one.
 */
export const POST = withUser<{ id: string }>(
  async (req, { user, params }) => {
    ...
    if (photos.length === 0) throw new ApiError(400, "Add at least one photo before re-analyzing", "no_photos");
    ...
    await audit({ userId: user.id, action: "item.reanalyze", entityType: "item", entityId: item.id, meta: { jobId: job.id, photos: photos.length }, ...requestMeta(req) });
    return json({ jobId: job.id, reused: false }, { status: 202 });
  },
  { rateLimit: { key: "reanalyze", limit: 20, windowSeconds: 600 } },
);
